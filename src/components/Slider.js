...
const Slide = ({
	...
	const clearOverrides = useMemo(() => (index === 0 || index === slides + 1) && {
		'data-qoverride': undefined
	}, [index]);
	const slideOverrides = {
		slide: { ...override('Slide', `Slide ${numb}`),
			...clearOverrides
		},
		image: { ...override('Slide Image', `Slide Image ${numb}`),
			...clearOverrides
		},
		content: { ...override('Slide Content', `Slide Content ${numb}`),
			...clearOverrides
		},
		head: { ...override('Slide Head', `Slide Head ${numb}`),
			...clearOverrides
		},
		text: { ...override('Slide Text', `Slide Text ${numb}`),
			...clearOverrides
		},
		link: { ...override('Slide Link', `Slide Link ${numb}`),
			...clearOverrides
		}
	};
	...
	const heightStyles = useMemo(() => height !== 'auto' && {
		'height': height,
		'min-height': height,
		'max-height': height
	}, [height]);
	return <Box {...slideOverrides.slide} {...widthStyles} {...heightStyles}>
		      
		<Image {...slideOverrides.image} />
		      
		{(showHead || showText || showLink) && <Box {...slideOverrides.content}>
...
const Arrow = ({
	...
	return <Box {...override('Arrow', `Arrow ${direction}`)} onClick={clickFunc}>
...
const Point = ({
	...
	return <Box {...override('Point', `Point ${numb}`, numb === active && 'Point :active')} onClick={clickEvent}>
		      
		<Icon {...override('Point Icon', `Point Icon ${numb}`, numb === active && 'Point Icon :active')} />
...
const CarouselComponent = ({
	...
	const [{
		...
		animate,
		lock
	}, setPositionState] = useState({
		active: 1,
		position: 100,
		animate: true,
		lock: false
	});
	const slides = useMemo(() => parseInt(slidesProp) > 0 ? parseInt(slidesProp) : 1, [slidesProp]);
	const duration = useMemo(() => parseFloat(durationProp) > 0 ? parseFloat(durationProp) * 1000 : 0, [durationProp]);
	const list = useMemo(() => [slides, ...Array.from({
		length: slides
	}, (v, i) => i + 1), 1], [slides]);
	...
	const handleResize = useCallback(el => {
		const sliderWidth = el instanceof Element ? el.offsetWidth : el[0].contentRect.width;
	...
	const clickNumb = useCallback(newActive => {
		if (lock) return;
		setSlide(newActive, 100 * newActive, true, false);
	}, [active, position, animate, lock]);
	return <Box {...rest} ref={sliderRef} overflow="hidden">
		      
		<Box {...override('Slides')} transform={`translateX(-${position}%)`} transition={`transform ${animate ? duration : 0}ms ${functionProp}`}>
			        
			{list.map((numb, index) => <Slide
				key={`slide-${index}`}
				index={index}
				slides={slides}
				numb={numb}
		...
		{showArrows && <Box {...override('Arrows')}>
			          
			<Arrow direction="Prev" clickFunc={clickPrev} override={override} />
			          
			<Arrow direction="Next" clickFunc={clickNext} override={override} />
			        
		</Box>}
		      
		{showDots && <Box {...override('Points')}>
			          
			{list.slice(1, -1).map((numb, idx) => <Point
				key={`point-${idx}`}
				numb={numb}
				active={active}
				clickFunc={clickNumb}
