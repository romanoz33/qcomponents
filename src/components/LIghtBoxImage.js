...
const LightboxImage = ({
	...
	const closeLightbox = useCallback(() => {
		...
		setZoom(false);
		if (offScrollProp) scroll.enable();
	}, [offLightboxProp, offScrollProp, isOpen]); // Фукция зума
	...
	return <Box {...rest}>
		<Box {...override('Wrapper user element', `Wrapper user element${offLightboxProp ? ':disabled' : ':active'}`)} onClick={openLightbox}>
		...
		<Box onClick={closeLightbox} {...override('Overlay', `Overlay${isOpen ? ':open' : ':close'}`)}>
			...
			<Image ref={imageRef} onClick={e => zoomImage(e)} {...override('Lightbox image', `Lightbox image${isOpen ? ':open' : ':close'}`, `Lightbox image zoom${isZoom ? isBigSize && ':in' : isBigSize && ':out'}`)} />
