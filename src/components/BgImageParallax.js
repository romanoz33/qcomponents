...
const BgImageParallax = ({
	...
	const scrollSpeed = useMemo(() => {
		...
		return !isNaN(numb) ? Math.abs(numb) : 1;
	}, [scrollSpeedProp]);
	const scrollInertia = useMemo(() => {
		const numb = parseInt(scrollInertiaProp);
	...
	const updateParallaxTop = () => {
		...
		const windowShift = windowHeight * (scrollSpeed > 1 ? scrollSpeed - 1 : 0);
