...
const regDate = /^(?:(?:31(\/|-|\.)(?:0?[13578]|1[02]))\1|(?:(?:29|30)(\/|-|\.)(?:0?[13-9]|1[0-2])\2))(?:(?:1[6-9]|[2-9]\d)?\d{2})$|^(?:29(\/|-|\.)0?2\3(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\d|2[0-8])(\/|-|\.)(?:(?:0?[1-9])|(?:1[0-2]))\4(?:(?:1[6-9]|[2-9]\d)?\d{2})$/; // Регулярка на проверку формата времени
// Принимает следующий формат 00:00

const regTime = /^(([0,1][0-9])|(2[0-3])):[0-5][0-9]$/; // Приведение значения к двузначному представленю

const addZero = num => num < 10 ? `0${num}` : num; // Получение оставшегося времени для вывода
...
const TimerComponent = ({
	...
	const isComplete = useMemo(() => showTextDone === 'complete' && isDone, [showTextDone, isDone]);
	...
	return <Box {...rest}>
		{!isAlways && !isComplete ? showList.map(item => item.show && <Box {...override('Item')}>
		...
		</Box>) : <Text {...override('Text')} />}
		{!showList.find(item => item.show) && <ComponentNotice message="Вы скрыли все выводимые значения таймера!" />}
	</Box>;
};
