...
const Accordion = ({
	...
	const onToggleOpen = ({
		...
		isPreventDoubleClick = !allowMultiple && !allowToggle && open && isOpenOneEl,
		      // Запрещать ли закрывать все элементы при отключенном allowMultiple
		isRequireLastElOpen = !allowToggle && open && isOpenOneEl,
		      // Выбираем действие, в зависимости от пропсов
		type = !open ? allowMultiple ? 'open' : 'toggle' : 'close'; // Если текущее действие нельзя выполнить
