...
const VKComments = ({
	...
	const key = useMemo(() => dApiId + JSON.stringify(dOptions) + dPageId + dElementId, [dApiId, dOptions, dPageId, dElementId]);
	return <Box {...props}>
		      
		{dApiId ? <VK key={key} apiId={dApiId}>
			            
			<Comments elementId={dElementId} pageId={dPageId} options={dOptions} />
			          
		</VK> : 'Добавьте ваш VK App ID на панели props'}
